...
def demo_fractal_memory():
    print("==============================================")
    print("   🧠 CMFO FRACTAL MEMORY DEMO (Associative Recall)")
    print("   Running on: Native C++ Engine (OpenMP)")
    print("==============================================\n")

    # Configuration
    MEMORY_SIZE = 100000  # Number of parallel timelines (superposition)
    EVOLUTION_STEPS = 50   # Depth of thought (iterations)
    
    # 0. Check Backend
    lib = NativeLib.get()
    if not lib:
        print("❌ CRITICAL: Native library not found. Demo requires OpenMP backend.")
        return

    print(f"[1] 初始化 Creating Superposition of {MEMORY_SIZE:,} states...")
