...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - CMFO - %(levelname)s - %(message)s')
...
class CMFORESTHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/health" or self.path == "/":
            self._set_headers()
            self.wfile.write(json.dumps({"status": "active", "system": "CMFO v1.0.0", "mode": "fractal"}).encode())
        else:
    ...
    def _handle_encode(self):
        try:
            ...
            response = {"text": data['text'], "vector": vec.tolist(), "dim": 7}
            self.wfile.write(json.dumps(response).encode())
