% CMFO_Calculo_Variacional_Fractal_Covariante_v1.3.tex
...
% ========== SECCIÓN 10: CÁLCULO DIFERENCIAL EN FRACTALES (FORMA DE DIRICHLET) ==========
\section{Cálculo diferencial intrínseco vía formas de Dirichlet}
\label{sec:dirichlet_intrinseco}

La Sección \ref{sec:calculo_variacional} define el operador $\Delta_A^{(\varphi)}$ en formulación débil a partir de derivadas covariantes.
...
Asumimos que $T^7_\varphi$ es un conjunto autosimilar que satisface OSC (Hipótesis \ref{ass:osc}) y admite una forma de resistencia
...
de la Definición \ref{def:laplaciano_covariante} coincide con $\Delta_\varphi$ al tomar $A\equiv 0$ y elegir la energía
...
(i) el error de discretización está acotado por una cota a priori (\S\ref{sec:implementacion});
...
En el esquema de Verlet de \S\ref{sec:conexion_cmfo}, los invariantes auditables mínimos son:
\[
...
\bibitem{HinzTeplyaev2013} Hinz, M., Teplyaev, A. (2013). Dirac and magnetic Schrödinger operators on fractals. \emph{J. Funct. Anal.}
