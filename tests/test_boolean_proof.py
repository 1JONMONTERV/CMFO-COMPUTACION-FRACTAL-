...
class TestBooleanAbsorption:
    ...
    def test_completeness_and(self):
        ...
        for a, b, expected in truth_table:
            ...
            assert from_cmfo(result) == expected, f"AND Error: {a}&{b}!={expected}"
    ...
    def test_completeness_or(self):
        ...
        for a, b, expected in truth_table:
            ...
            assert from_cmfo(result) == expected, f"OR Error: {a}|{b}!={expected}"
    ...
    def test_completeness_xor(self):
        ...
        for a, b, expected in truth_table:
            ...
            assert from_cmfo(result) == expected, f"XOR Error: {a}^{b}!={expected}"
    ...
    def test_completeness_nand(self):
        ...
        for a, b, expected in truth_table:
            ...
            assert from_cmfo(result) == expected, f"NAND Error: {a} NAND {b}!={expected}"

    def test_continuity_hypothesis(self):
        """
        Verifica que el operador funciona incluso con ruido (valores no exactos).
        Esto demuestra la robustez del axioma de continuidad.
        """
        # 0.8 es "Casi Verdad" (True)
        # -0.9 es "Falsedad Profunda" (False)
